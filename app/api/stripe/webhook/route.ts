  ...
  try {
    switch (event.type) {
      ...
      case "customer.subscription.deleted": {
        ...
        const locked = status === "canceled" || status === "unpaid" || status === "incomplete_expired";
        const currentPeriodEnd = subscription?.current_period_end
          ? new Date(subscription.current_period_end * 1000).toISOString()
          : null;
        ...
        if (data?.auth_user_id) {
          ...
          if (locked && data.email) await sendPlanLockedEmail(data.email);
        }
        break;
      }
      ...
      case "invoice.payment_failed": {
        ...
        if (data?.auth_user_id) {
          ...
          if (data.email) await sendPaymentFailedEmail(data.email);
        }
        break;
      }
    }
